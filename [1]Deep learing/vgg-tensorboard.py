...
def variable_summary(var,name):
    # 给一个变量很多统计量，建立一个summary，这些summary就可以输出到直方图中
...
# 第二步：只需要指定到文件夹即可
LOG_DIR = '.'
run_label = 'run_vgg_tensorboard' # 创建文件夹
...
with tf.Session() as sess: # 是一个会话的界面，执行这个计算图
    ...
    for i in range(train_steps):
        ...
        eval_ops_results= sess.run(eval_ops,feed_dict={x: batch_data,y: batch_labels})  #塞入输入  feed_dict={x:,y:} 图片数据和label数据
        loss_val, acc_val = eval_ops_results[0:2]
        
        if should_output_summary:
            train_summary_str = eval_ops_results[-1]
            train_writer.add_summary(train_summary_str,i+1)# 指定的步数
